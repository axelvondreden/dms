...
class PdfToDocParser(
    ...
    private val eventListeners = HashMap<String, ParserEventListener>()
    ...
    override fun parse(file: File?) {
        ...
        if (savedFile.isPresent) {
            ...
            eventListeners.values.forEach { it.onParse(true) }
        } else {
            LOGGER.error("Could not save file {}", file.absolutePath)
            eventListeners.values.forEach { it.onParse(false) }
        }
    }
    ...
    private fun discoverTags(rawText: String?): Set<Tag> {
        ...
        if (OptionKey.AUTO_TAG.boolean) {
            tagService.findById(OptionKey.AUTO_TAG_ID.long).ifPresent {
    ...
    fun addEventListener(key: String, eventListener: ParserEventListener) {
