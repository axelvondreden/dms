...
class RulesView(
    ...
    private fun addPlaintext() {
        ...
        plainTextRuleService.activeRules.map { builderFactory.rules().plainTextCard(it, { fillContent() }) { fillContent() }.build() }.forEach { verticalLayout.add(it) }
    ...
    private fun addRegex() {
        ...
        val verticalLayout = VerticalLayout(create).apply { setSizeFull() }
        regexRuleService.activeRules.map { builderFactory.rules().regexCard(it, { fillContent() }) { fillContent() }.build() }.forEach { verticalLayout.add(it) }
