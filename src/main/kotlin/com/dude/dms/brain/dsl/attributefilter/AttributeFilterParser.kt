...
class AttributeFilterParser {

    data class ParseResult(val attributeFilter: AttributeFilterLang.Query?, val error: String?, val isValid: Boolean)
    ...
    fun setInput(text: String): ParseResult {
        this.text = text
        return try {
            ParseResult(AttributeFilterLang.parse(text.trim()), null, true)
    ...
    fun getHints(): HintResult {
        ...
        val queryTest = AttributeFilterLang.testForQueryDroppingLast(text.trim())
        ...
        val opTest = AttributeFilterLang.testForOpFromEnd(text.trim())
        if (opTest.first != null) {
            val keyTest = AttributeFilterLang.testForKeyFromEnd(text.trim().dropLast(opTest.second))
        ...
        val keyTest = AttributeFilterLang.testForKeyFromEnd(text.trim())
        if (keyTest.first != null) {
